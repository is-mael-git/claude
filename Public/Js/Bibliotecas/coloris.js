...
((window, document, Math, undefined) => {
    ...
    for (const key in options) {
      switch (key) {
        ...
        case 'rtl':
          ...
          Array.from(document.getElementsByClassName('selecionarCor')).forEach(field => field.classList.toggle('clr-rtl', settings.rtl));
          break;
        case 'margin':
          options.margin *= 1;
  ...
  function wrapColorField(field) {
    ...
    if (!parentNode.classList.contains('selecionarCor')) {
      const wrapper = document.createElement('div');
      let classes = 'selecionarCor';
  ...
  function updateColorPreview(event) {
    ...
    if (parent.classList.contains('selecionarCor')) {
  ...
  function init() {
    ...
    addListener(document, 'click', '.selecionarCor button', event => {
      // Reset any previously set per-instance options
      if (hasInstance) {
        resetVirtualInstance();
      }
